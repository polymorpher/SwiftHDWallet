...
//  
//
//  Created by Ronald Mannak on 10/8/20.
//
// https://github.com/LN-Zap/SwiftBTC/blob/master/SwiftBTC/Classes/Bech32.swift
//
//  SwiftBTC
//
...
public enum Bech32Zap {
    ...
    private static func polymod(values: Data) -> Int {
        ...
        for p in values {
            ...
            // swiftlint:disable:next identifier_name
            for (i, g) in Bech32Zap.generator.enumerated() where ((top >> i) & 1) != 0 {
                chk ^= g
            }
        }
        return chk
    ...
    //private
    static func createChecksum(humanReadablePart: String, data: Data) -> Data {
...
//            guard let distance = Bech32Zap.alphabet.indexDistance(of: character) else { return nil }
...
//    }
    ...
    //private
    static func toChars(data: Data) -> String {
        return data.reduce("") {
            let index = Bech32Zap.alphabet.index(Bech32Zap.alphabet.startIndex, offsetBy: Int($1))
            return $0 + String(Bech32Zap.alphabet[index])
        }
    }

    public static func encode(humanReadablePart: String, data: Data) -> String {
        let checksum = Bech32Zap.createChecksum(humanReadablePart: humanReadablePart, data: data)
        let combined = data + checksum

        return humanReadablePart + "1" + Bech32Zap.toChars(data: combined)
    }
}
